...
class _ExecConnection(psycopg2.extensions.connection):
    """Adds sqlite3-style `conn.execute(...) -> cursor` for minimal call-site changes."""
...
def get_db():
    if "db" not in g:
        g.db = psycopg2.connect(DATABASE_URL, cursor_factory=RealDictCursor,
                                 connection_factory=_ExecConnection)
    return g.db
...
def get_member(mid):
    if not mid:
        return None
    return to_dict(get_db().execute("SELECT * FROM members WHERE id=%s", (mid,)).fetchone())

def get_book(bid):
    if not bid:
        return None
    return to_dict(get_db().execute("SELECT * FROM books WHERE id=%s", (bid,)).fetchone())
...
def get_meals_for_book(book_id):
    return to_dicts(get_db().execute(
        "SELECT * FROM meals WHERE book_id=%s", (book_id,)
    ).fetchall())
...
def template_helpers():
    def avatar_url(member, bg="1a1826"):
        ...
        if img:
            return f"{SUPABASE_URL}/storage/v1/object/public/{AVATAR_BUCKET}/{img}"
...
def enrich_book(book):
    ...
    rating_rows = get_db().execute(
        "SELECT * FROM ratings WHERE book_id=%s", (book["id"],)
    ).fetchall()
...
def enrich_member(member):
    ...
    member["books_selected_list"] = to_dicts(
        db.execute(
            "SELECT * FROM books WHERE selected_by=%s ORDER BY date_read", (member["id"],)
        ).fetchall()
    )
    member["meals_cooked_list"] = [
        enrich_meal(m) for m in to_dicts(
            db.execute("SELECT * FROM meals WHERE prepared_by=%s", (member["id"],)).fetchall()
        )
    ]

    rating_rows = db.execute(
        "SELECT book_id, score, review FROM ratings WHERE member_id=%s", (member["id"],)
    ).fetchall()
    ratings_given = []
    for r in rating_rows:
...
def meal_detail(meal_id):
    meal = to_dict(get_db().execute("SELECT * FROM meals WHERE id=%s", (meal_id,)).fetchone())
    if not meal:
        abort(404)
...
def book_new():
    if request.method == "POST":
        ...
        cur = db.execute("""
            ...
            VALUES
                (%(title)s, %(author)s, %(genre)s, %(pages)s, %(goodreads_rating)s, %(year_published)s,
                 %(date_read)s, %(selected_by)s, %(description)s, %(cover_color)s, %(cover_accent)s,
                 %(tags)s, %(event_note)s, %(goodreads_url)s)
            RETURNING id
        """, f)
...
def book_edit(book_id):
    ...
    if request.method == "POST":
        ...
        get_db().execute("""
            UPDATE books SET
                title=%(title)s, author=%(author)s, genre=%(genre)s, pages=%(pages)s,
                goodreads_rating=%(goodreads_rating)s, year_published=%(year_published)s,
                date_read=%(date_read)s, selected_by=%(selected_by)s, description=%(description)s,
                cover_color=%(cover_color)s, cover_accent=%(cover_accent)s, tags=%(tags)s,
                event_note=%(event_note)s, goodreads_url=%(goodreads_url)s
            WHERE id=%(id)s
        """, f)
        get_db().commit()
...
def book_delete(book_id):
    ...
    db = get_db()
    db.execute("DELETE FROM books WHERE id=%s", (book_id,))  # meals/ratings cascade
    db.commit()
...
def rating_delete(book_id, member_id):
    if not get_book(book_id):
        abort(404)
    db = get_db()
    db.execute("DELETE FROM ratings WHERE book_id=%s AND member_id=%s", (book_id, member_id))
    db.commit()
...
def person_new():
    if request.method == "POST":
        ...
        cur = db.execute("""
            ...
            VALUES
                (%(name)s, %(nickname)s, %(avatar_seed)s, %(fun_facts)s, %(quotes)s,
                 %(favorite_genre)s, %(join_date)s, %(signature_dish)s)
            RETURNING id
        """, f)
        new_id = cur.fetchone()["id"]
        db.commit()
        ...
        if filename:
            db.execute("UPDATE members SET avatar_img=%s WHERE id=%s", (filename, new_id))
            db.commit()
...
def person_edit(member_id):
    ...
    if request.method == "POST":
        ...
        db.execute("""
            UPDATE members SET
                name=%(name)s, nickname=%(nickname)s, avatar_seed=%(avatar_seed)s,
                fun_facts=%(fun_facts)s, quotes=%(quotes)s, favorite_genre=%(favorite_genre)s,
...
def person_delete(member_id):
    ...
    db = get_db()
    db.execute("DELETE FROM members WHERE id=%s", (member_id,))
    db.commit()
...
def meal_new():
    if request.method == "POST":
        ...
        cur = db.execute("""
            ...
            VALUES (%(book_id)s, %(dish)s, %(prepared_by)s, %(type)s, %(description)s, %(recipe)s)
            RETURNING id
        """, f)
        new_id = cur.fetchone()["id"]
        db.commit()
        flash(f"'{f['dish']}' added!", "success")
        return redirect(url_for("meal_detail", meal_id=new_id))
...
def meal_edit(meal_id):
    ...
    if request.method == "POST":
        ...
        get_db().execute("""
            UPDATE meals SET book_id=%(book_id)s, dish=%(dish)s, prepared_by=%(prepared_by)s,
                type=%(type)s, description=%(description)s, recipe=%(recipe)s
            WHERE id=%(id)s
        """, f)
        get_db().commit()
...
def meal_delete(meal_id):
    ...
    db = get_db()
    db.execute("DELETE FROM meals WHERE id=%s", (meal_id,))
    db.commit()
